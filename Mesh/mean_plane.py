...
class Ransac:
    ...
    def fit(self,X,z,max_trials = 130,min_samples = 0.8,stop_probability = 0.99):
        ...
        self.line_X        = np.arange(X.min(), X.max())[:, np.newaxis]
        self.line_z_ransac = ransac.predict(self.line_X)
        
        return self.line_X,self.line_z_ransac
    ...
    def get_quartenion(self):
        ...
        u = self.normal
        v = self.normalize(np.array([0.51,0,-0.49]))
...
for k in range(3,37):
    ...
    RANSAC                = Ransac()
    line_X, line_z_ransac = RANSAC.fit(data.positions[:,0],data.positions[:,2])
    ...
    for position,quaternion in zip(data.positions,data.rotations):
        x,y,z,r = position
        logs.append(Log(x,y,z,r,mean_plane_quartenion))


    #R = arrow.get_rotation_matrix_from_xyz((np.pi / 2, 0, np.pi / 4))
