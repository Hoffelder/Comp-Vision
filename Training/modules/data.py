...
class Tflite:
    ...
    def generate_train_parameters_file(self):
        ...
        #    "epochs": [2,3,4],
        #    "batch_size": [8,16,32],
        #    "steps_per_execution": [1,10],
        #    "tflite_max_detections": [100,500,1000],
        #    "var_freeze_expr": ['(efficientnet|fpn_cells|resample_p6)','(NONE|NONE|NONE)']
        #}
        ...
        file_exists = exists("hiper_parameters/training.json")
        if not file_exists:
            with open("hiper_parameters/training.json", "w") as outfile:
                json_object = json.dumps(training,indent=4)
                outfile.write(json_object)

    def read_train_parameters_file(self):
        with open("hiper_parameters/training.json", "r") as openfile:
